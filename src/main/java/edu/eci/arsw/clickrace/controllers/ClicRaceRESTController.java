...
public class ClicRaceRESTController {
    ...
    public ResponseEntity<?> getRaceParticipantsNums(@PathVariable(name = "racenum") String racenum) {
        ...
        try {
            Set<RaceParticipant> temp = services.getRegisteredPlayers(Integer.parseInt(racenum));
            if(temp.size()== 5)return new ResponseEntity<>(temp,HttpStatus.ACCEPTED);
            else return new ResponseEntity<>("Missing racers",HttpStatus.BAD_REQUEST);
    ...
    @GetMapping("/{racenum}/winner")
    public ResponseEntity<?> getWinner(@PathVariable(name = "racenum") String racenum) {
        try {
            return ResponseEntity.ok(services.getWinner(Integer.parseInt(racenum)));
        } catch (NumberFormatException e) {
            Logger.getLogger(ClicRaceRESTController.class.getName()).log(Level.SEVERE, null, e);
            return new ResponseEntity<>("/{racenum}/ must be an integer value.",HttpStatus.BAD_REQUEST);
